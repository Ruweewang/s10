...
def insert_comment_node(data_dic,margin_val):
    ...
    for p,v in data_dic.items():
        r = '''
            <div style="margin-left:%spx;border-lefet:1px dashed green;border-bottom:1px dashed green">
            <span class="comment-user">%s</span>
            <span class="comment-content">%s</span>
            <span class="comment-date">%s</span>
            </div>
        '''%(margin_val,p.user.name,p.comment,p.date)
...
def build_comment_tree(tree_data):
    ...
    for p,v in tree_data.items():
        row = '''
            <div style="margin_top:15px;border-lefet:1px dashed green;border-bottom:1px dashed green">
            <span class="comment-user">%s</span>
            <span class="comment-content">%s</span>
            <span class="comment-date">%s</span>
            </div>
        '''%(p.user.name,p.comment,p.date)
        if v is not None:
